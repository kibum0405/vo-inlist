<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            InList # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            InList
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <Money offline label="Mony" v-model="value.mony" :editMode="editMode" @change="change"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/inLists'), this.value)
                        } else {
